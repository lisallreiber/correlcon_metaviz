...
This markdown outlines the process in which synthetic SOEP-IS data is
...
```{r import data}
metaviz <- rio::import(here::here("data/metaviz.rds"))
key_labels <- rio::import(here::here("data/key_labels.rds"))
```
...
now we create an additional dataset which is long in the sense that all variables are represented in the `key`column and all corresponding values in a `value` column. 
...
```{r}
rio::export(metaviz_long, here::here("data", "metaviz_long.rds"))
```
